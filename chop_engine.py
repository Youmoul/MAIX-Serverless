...
# Original audio is NOT resampled for the exported slices.
...
def _analysis_signal(waveform, sample_rate):
    ...
    # Normalize analysis signal only.
    peak = mono.abs().max()
...
def _spectral_flux(signal):
    ...
    Strong positive changes in spectral energy are good candidates for
    musical attacks/transients: drums, piano attacks, chord attacks, etc.
    """
    ...
    difference = magnitude[:, 1:] - magnitude[:, :-1]

    # Only increases in spectral energy matter for onset detection.
...
def _find_peak_candidates(flux):
    ...
    # Ignore very weak activity.
    mean = flux.mean()
    std = flux.std()

    threshold = float(mean + (0.5 * std))
...
def _select_boundaries(
    ...
    """
    Select the strongest peaks while guaranteeing that every resulting
    slice is at least min_slice_seconds long.

    Candidate peaks are considered strongest-first.

    Once selected, boundaries are sorted chronologically.
    """
    if duration <= min_slice_seconds:
        return [0.0, duration]

    # Number of internal boundaries needed for N slices.
    ...
    for peak_time, strength in candidates:

        # MAIX rule:
        # cut 2 milliseconds BEFORE the detected peak.
        cut_time = max(
            0.0,
            peak_time - pre_peak_offset,
        )

        # Do not create a tiny first slice.
        if cut_time < min_slice_seconds:
            continue

        # Do not create a tiny final slice.
        if duration - cut_time < min_slice_seconds:
            continue

        # Cut must also remain at least 2 sec away from every
        # previously accepted cut.
        too_close = any(
            abs(cut_time - existing) < min_slice_seconds
    ...
    for boundary in boundaries[1:-1]:
        if boundary - safe[-1] >= min_slice_seconds:
            safe.append(boundary)

    # If the last selected boundary would leave a short ending,
    # remove boundaries until the ending is valid.
    while (
        len(safe) > 1
        and duration - safe[-1] < min_slice_seconds
    ):
        safe.pop()
...
def intelligent_chop(
    ...
    """
    Intelligently chop an input song around its strongest transients.
    ...
    if duration < min_slice_seconds:
        raise ValueError(
            ...
            f"{min_slice_seconds:.1f} seconds long."
        )

    max_possible_slices = max(
        1,
        int(math.floor(duration / min_slice_seconds)),
    )
    ...
    flux = _spectral_flux(analysis)

    candidates = _find_peak_candidates(flux)
